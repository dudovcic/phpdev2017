...
-- Dumping data for table diskovi.posudba: ~2 rows (approximately)
...
-- Dumping structure for view diskovi.v_trenutnostanje
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `v_trenutnostanje` (
	`ime` CHAR(50) NULL COLLATE 'utf8_general_ci',
	`prezime` CHAR(50) NULL COLLATE 'utf8_general_ci',
	`izvodjac` CHAR(50) NULL COLLATE 'utf8_general_ci',
	`tip_diska` CHAR(50) NULL COLLATE 'utf8_general_ci',
	`godina_proizvodnje` CHAR(50) NULL COLLATE 'utf8_general_ci',
...
DROP TABLE IF EXISTS `v_trenutnostanje`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `v_trenutnostanje` AS select imena.ime,
 imena.prezime,
  cd_dvd.izvodjac,
   cd_dvd.tip_diska, 
	cd_dvd.godina_proizvodnje, 
	posudba.datum_posudbe,
	 posudba.datum_vracanja,
	  datediff (posudba.datum_vracanja, posudba.datum_posudbe) as brojdana


 from imena
inner join posudba on posudba.Korisnik=imena.id_i
inner join cd_dvd on cd_dvd.izvodjac=posudba.Izvodjac ;
