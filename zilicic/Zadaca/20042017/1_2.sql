...
CREATE TABLE IF NOT EXISTS `proizvod` (
  ...
  KEY `skl_FK` (`sifra_skladista`),
  CONSTRAINT `skl_FK` FOREIGN KEY (`sifra_skladista`) REFERENCES `skladisno_mjesto` (`sifra_skladista`)
...
CREATE TABLE IF NOT EXISTS `voditelj` (
  ...
  KEY `FK_voditelj_skladiste` (`sifra_skladista`),
  CONSTRAINT `FK_voditelj_skladiste` FOREIGN KEY (`sifra_skladista`) REFERENCES `skladisno_mjesto` (`sifra_skladista`),
  CONSTRAINT `FK_voditelj_zaposlenici` FOREIGN KEY (`sifra_zaposlenik`) REFERENCES `zaposlenici` (`sifra_zaposlenik`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `zaposlenici` (
  ...
  KEY `zap_FK` (`sifra_skladista`),
  CONSTRAINT `zap_FK` FOREIGN KEY (`sifra_skladista`) REFERENCES `skladisno_mjesto` (`sifra_skladista`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Dumping data for table skladiste.zaposlenici: ~3 rows (approximately)
